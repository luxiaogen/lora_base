#!/usr/bin/env bash
set -uo pipefail

cd "$(dirname "$0")"

LOG_DIR="logs/shell_logs/task0_label_smoothing"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
START_TIME=$(date +%s)
SEED=1993
EPSILON=0.05
MODES=(protect_only unmasked)
mkdir -p "$LOG_DIR"

run_experiment() {
    local dataset="$1"
    local config="$2"
    local mode="$3"
    local prefix="${dataset}_task0_${mode}_ls005_seed${SEED}"
    local log_file="${LOG_DIR}/${prefix}_${TIMESTAMP}.log"
    local run_start
    local run_end
    local run_seconds

    echo "============================================================"
    echo "Starting ${dataset}, seed=${SEED}, task0_gate_mode=${mode}"
    echo "Task 0 label smoothing=${EPSILON}; Task >= 1 label smoothing=0"
    echo "Task 0: ${mode}; Task >= 1: full DualMask"
    echo "Log: ${log_file}"
    echo "============================================================"

    run_start=$(date +%s)
    if python main.py \
        --config "$config" \
        --set "seed=[${SEED}]" \
        --set "prefix=${prefix}" \
        --set init_epoch=20 \
        --set epochs=20 \
        --set ca=true \
        --set ca_epochs=10 \
        --set ca_lrate=0.01 \
        --set dual_mask_importance=svd \
        --set dual_mask_svd_rank=768 \
        --set dual_mask_svd_energy_coverage=0.95 \
        --set dual_mask_competence_adaptive=true \
        --set dual_mask_competence_all_seen=false \
        --set dual_mask_competence_metric=accuracy \
        --set dual_mask_plasticity_adaptive=true \
        --set dual_mask_conflict_ratio=0.1 \
        --set dual_mask_conflict_strength=0.5 \
        --set dual_mask_conflict_old_overlap_adaptive=true \
        --set "dual_mask_task0_gate_mode=${mode}" \
        --set "label_smoothing=${EPSILON}" \
        --set label_smoothing_task0_only=true \
        --set dual_mask_track_w0_metrics=true \
        --set dual_mask_vis=false \
        --set experiment_tracker=wandb \
        --set wandb_project=LoDA_ICML2026 \
        --set wandb_mode=online \
        --set "wandb_group=task0_label_smoothing_${TIMESTAMP}" \
        --set "wandb_tags=task0_label_smoothing,eps005,${dataset},${mode},seed${SEED}" \
        2>&1 | tee "$log_file"; then
        echo "Finished ${prefix}"
    else
        echo "FAILED ${prefix}; continuing with remaining experiments." >&2
    fi

    run_end=$(date +%s)
    run_seconds=$((run_end - run_start))
    printf 'Duration: %ds (%dh %dm %ds)\n' \
        "$run_seconds" \
        "$((run_seconds / 3600))" \
        "$(((run_seconds % 3600) / 60))" \
        "$((run_seconds % 60))"
}

for dataset in cub10 imgr10 imga10; do
    case "$dataset" in
        cub10) config="ideas/dual_mask_branch/configs/cub10.json" ;;
        imgr10) config="ideas/dual_mask_branch/configs/imgr10.json" ;;
        imga10) config="ideas/dual_mask_branch/configs/imga10.json" ;;
    esac

    for mode in "${MODES[@]}"; do
        run_experiment "$dataset" "$config" "$mode"
    done
done

END_TIME=$(date +%s)
TOTAL_SECONDS=$((END_TIME - START_TIME))

echo "============================================================"
echo "Finished 6 Task-0 label-smoothing experiments."
printf 'Total time: %ds (%dh %dm %ds)\n' \
    "$TOTAL_SECONDS" \
    "$((TOTAL_SECONDS / 3600))" \
    "$(((TOTAL_SECONDS % 3600) / 60))" \