...
class Learner(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        if prepare_w0 or bool(self.args.get("dual_mask_p_region_diagnostic", False)) or bool(self.args.get("dual_mask_p_conflict_group_diagnostic", False)):
        ...
        if bool(self.args.get("dual_mask_p_conflict_group_diagnostic", False)):
            if self._cur_task in (1, 2, 9):
                self._diagnose_p_conflict_groups(self.test_loader.dataset, "post_ca_test_diagnostic_only")
            for module in self._iter_lora_modules():
                module._p_conflict_component = None
    ...
    def _diagnose_p_conflict_groups(self, dataset, source):
        from torch.utils.data import Subset
        from utils.p_conflict_groups import balanced_indices, compare_groups
        # Fixed deterministic test transform, no old training data retention, no gate selection.
        indices = balanced_indices(dataset.labels, per_class=4)
        loader = DataLoader(Subset(dataset, indices), batch_size=self.batch_size, shuffle=False, num_workers=0)
        report = compare_groups(self._network, list(self._iter_lora_modules()), loader, self._device, self.task_sizes)
        logging.info("P-conflict groups %s", json.dumps({"task": self._cur_task, "source": source,
                     "current_task_component_only": True, "selection_used": False,
                     "positive_margin_change_means": "removal_helped", "report": report}, allow_nan=False))
    ...
    def _train(self, train_loader, test_loader):
        ...
        if bool(self.args.get("dual_mask_p_conflict_group_diagnostic", False)) and self._cur_task in (1, 2, 9):
            self._diagnose_p_conflict_groups(self.w0_loader.dataset, "pre_ca_train_subset_not_holdout")
