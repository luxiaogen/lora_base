...
# --smoke runs only the focused unit test.
case "${1:-}" in
    ""|--smoke) ;;
...
PYTHONWARNINGS=ignore python -m unittest \
    test.test_ca_diagnostics.CADiagnosticsTests.test_ca_mean_decay_can_be_disabled_without_changing_default || exit 1
if [[ "${1:-}" == "--smoke" ]]; then
    exit 0
fi
echo "Code revision: $(git rev-parse --short HEAD)"
LOG_DIR=logs/shell_logs/imgr10_ca_mean_decay_seed1993
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
FAILED=0
mkdir -p "$LOG_DIR"

echo "============================================================"
echo "Starting imgr10_current_decay_seed1993"
echo "Changed: Current CA class-mean decay schedule"
echo "Log: $LOG_DIR/imgr10_current_decay_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set prefix=imgr10_current_decay_seed1993 \
        --set init_epoch=20 \
        --set epochs=20 \
        --set rank=64 \
        --set ca=true \
        ...
        --set wandb_group=imgr10_ca_mean_decay_seed1993 \
        --set ca_mean_decay_enabled=true \
        2>&1 | tee "$LOG_DIR/imgr10_current_decay_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_current_decay_seed1993"
else
    echo "FAIL imgr10_current_decay_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Starting imgr10_no_decay_seed1993"
echo "Changed: CA class means are not scaled"
echo "Log: $LOG_DIR/imgr10_no_decay_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set prefix=imgr10_no_decay_seed1993 \
        --set init_epoch=20 \
        --set epochs=20 \
        --set rank=64 \
        --set ca=true \
        ...
        --set wandb_mode=online \
        --set wandb_group=imgr10_ca_mean_decay_seed1993 \
        --set ca_mean_decay_enabled=false \
        2>&1 | tee "$LOG_DIR/imgr10_no_decay_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_no_decay_seed1993"
else
    echo "FAIL imgr10_no_decay_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Finished 2 runs for imgr10_ca_mean_decay_seed1993; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
exit $FAILED
