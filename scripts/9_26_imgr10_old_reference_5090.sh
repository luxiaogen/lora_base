...
if [[ "${1:-}" == "--smoke" ]]; then
    shift
    exec bash scripts/9_26_imgr10_old_competition_5090.sh --smoke "$@" --set old_competition_detach_old=true
fi
...
python scripts/night_mechanism_preflight.py scripts/sweeps/imgr10_old_reference_5090.json || exit 1
echo "Code revision: $(git rev-parse --short HEAD)"
git status --short
LOG_DIR=logs/shell_logs/imgr10_old_reference_5090
...
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_old_reference_seed1993_5090_${TIMESTAMP}" \
        ...
        --set wandb_group=imgr10_old_reference_5090 \
...
echo "============================================================"
echo "Finished 1 runs for imgr10_old_reference_5090; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
if [[ "$FAILED" -eq 0 ]]; then
