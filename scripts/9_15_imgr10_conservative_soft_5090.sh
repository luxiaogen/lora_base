...
python -m unittest test.test_p_conflict_diagnostics test.test_qv_all_tasks || exit 1
...
git status --short
LOG_DIR=logs/shell_logs/imgr10_conservative_soft_full_t10_5090
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
FAILED=0
mkdir -p "$LOG_DIR"

echo "============================================================"
echo "Starting imgr10_qkv_conservative_seed1993"
echo "Changed: Fresh QKV baseline with same-model ones, soft, and entropy-conservative P diagnostics"
echo "Log: $LOG_DIR/imgr10_qkv_conservative_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set prefix=imgr10_qkv_conservative_seed1993 \
        --set max_tasks=10 \
        ...
        --set wandb_group=imgr10_conservative_soft_full_t10_5090 \
        --set wandb_tags=imgr10,t10,qkv,p_conflict_conservative,inference_only,multiseed \
        2>&1 | tee "$LOG_DIR/imgr10_qkv_conservative_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_qkv_conservative_seed1993"
else
    echo "FAIL imgr10_qkv_conservative_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Starting imgr10_qkv_conservative_seed1996"
echo "Changed: Fresh QKV baseline with same-model ones, soft, and entropy-conservative P diagnostics"
echo "Log: $LOG_DIR/imgr10_qkv_conservative_seed1996_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1996]' \
        --set prefix=imgr10_qkv_conservative_seed1996 \
        --set max_tasks=10 \
        --set total_sessions=10 \
        --set init_cls=20 \
        --set increment=20 \
        ...
        --set wandb_group=imgr10_conservative_soft_full_t10_5090 \
        --set wandb_tags=imgr10,t10,qkv,p_conflict_conservative,inference_only,multiseed \
        2>&1 | tee "$LOG_DIR/imgr10_qkv_conservative_seed1996_${TIMESTAMP}.log"
then
    echo "PASS imgr10_qkv_conservative_seed1996"
else
    echo "FAIL imgr10_qkv_conservative_seed1996"
    FAILED=1
fi

echo "============================================================"
echo "Starting imgr10_qkv_conservative_seed1997"
echo "Changed: Fresh QKV baseline with same-model ones, soft, and entropy-conservative P diagnostics"
echo "Log: $LOG_DIR/imgr10_qkv_conservative_seed1997_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1997]' \
        --set prefix=imgr10_qkv_conservative_seed1997 \
        --set max_tasks=10 \
        --set total_sessions=10 \
        --set init_cls=20 \
        --set increment=20 \
        ...
        --set wandb_group=imgr10_conservative_soft_full_t10_5090 \
        --set wandb_tags=imgr10,t10,qkv,p_conflict_conservative,inference_only,multiseed \
        2>&1 | tee "$LOG_DIR/imgr10_qkv_conservative_seed1997_${TIMESTAMP}.log"
then
    echo "PASS imgr10_qkv_conservative_seed1997"
else
    echo "FAIL imgr10_qkv_conservative_seed1997"
    FAILED=1
fi

echo "============================================================"
echo "Finished 3 runs for imgr10_conservative_soft_full_t10_5090; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
exit $FAILED
