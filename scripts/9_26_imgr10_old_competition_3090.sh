...
if [[ "${1:-}" == "--smoke" ]]; then
    ...
    python -m unittest test.test_old_competition test.test_stage_audit || exit 1
    bash "$SCRIPT_PATH" --smoke "$@" || exit 1
fi
python scripts/night_mechanism_preflight.py scripts/sweeps/imgr10_old_competition_3090.json || exit 1
echo "Code revision: $(git rev-parse --short HEAD)"
git status --short
LOG_DIR=logs/shell_logs/imgr10_old_competition_3090
...
echo "Changed: anchor5 baseline; error partition audit"
...
if
    python main.py --config exps/dlora/imgr10.json \
        ...
        --set wandb_group=imgr10_old_competition_3090 \
...
echo "============================================================"
echo "Starting imgr10_t10_competition_seed1993"
echo "Changed: only add fixed 0.1 old-class competition hinge"
echo "Log: $LOG_DIR/imgr10_t10_competition_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_competition_seed1993_3090_${TIMESTAMP}" \
        --set dataset=ImageNet_R \
        --set memory_size=0 \
        --set memory_per_class=0 \
        --set fixed_memory=true \
        ...
        "$@" \
        2>&1 | tee "$LOG_DIR/imgr10_t10_competition_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_t10_competition_seed1993"
else
    echo "FAIL imgr10_t10_competition_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Finished 2 runs for imgr10_old_competition_3090; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
if [[ "$FAILED" -eq 0 ]]; then
    python scripts/summarize_stage_audit.py --tasks "$TASKS" --output "$LOG_DIR/stages_${TIMESTAMP}.csv" \
        ...
        "$LOG_DIR/imgr10_t10_competition_seed1993_${TIMESTAMP}.log" || FAILED=1
fi
if [[ "$RUN_KIND" == smoke && "$FAILED" -eq 0 ]]; then
    python scripts/check_old_competition_smoke.py \
        "$LOG_DIR/imgr10_t10_baseline_seed1993_${TIMESTAMP}.log" \
        "$LOG_DIR/imgr10_t10_competition_seed1993_${TIMESTAMP}.log" || FAILED=1
fi
exit $FAILED
