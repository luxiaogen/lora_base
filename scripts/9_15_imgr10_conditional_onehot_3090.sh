...
git status --short
LOG_DIR=logs/shell_logs/imgr10_conditional_onehot_3090
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
FAILED=0
mkdir -p "$LOG_DIR"

echo "============================================================"
echo "Starting imgr10_conditional_onehot_seed1993"
echo "Changed: Fresh QKV; predicted one-hot only when task-probability margin is below conflict_ratio=0.1"
echo "Log: $LOG_DIR/imgr10_conditional_onehot_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set prefix=imgr10_conditional_onehot_seed1993 \
        --set max_tasks=10 \
        ...
        --set wandb_group=imgr10_conditional_onehot_3090 \
        --set wandb_tags=imgr10,t10,qkv,seed1993,conditional_onehot,margin01,inference_only \
        2>&1 | tee "$LOG_DIR/imgr10_conditional_onehot_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_conditional_onehot_seed1993"
else
    echo "FAIL imgr10_conditional_onehot_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Finished 1 runs for imgr10_conditional_onehot_3090; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
exit $FAILED
