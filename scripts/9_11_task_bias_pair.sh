...
# Run the same command on both machines. Each machine reads its own data_path
# from exps/dlora/imgr10.json.
[[ -f main.py ]] || { echo 'Run from the repository root.' >&2; exit 2; }
PYTHON_BIN=${PYTHON_BIN:-python}
export PYTHONUNBUFFERED=1
tests=(test.test_task_bias_calibration test.test_task0_checkpoint test.test_ca_diagnostics test.test_task_bias_script)
if [[ ${1:-} == --smoke ]]; then
...
[[ $# == 0 ]] || { echo 'Usage: bash scripts/9_11_task_bias_pair.sh [--smoke|--dry-run]' >&2; exit 2; }
...
if $dry_run; then
    run_dir=logs/shell_logs/task_bias_DRY_RUN
else
    ...
    run_dir=$(mktemp -d "logs/shell_logs/task_bias_$(date +%Y%m%d_%H%M%S)_XXXXXX")
fi
...
common=(--config exps/dlora/imgr10.json
    ...
    --set "wandb_mode=${WANDB_MODE:-online}" --set wandb_group=imgr10_task_bias_pair
    --set wandb_tags=imgr10,task_bias,paired,ca5,partial_task2)
...
for seed in "${seeds[@]}"; do
    ...
    run_stage "imgr10_task_bias_task0_seed${seed}" --set dual_mask_task_bias_calibration=false \
        --set max_tasks=1 --set "task0_checkpoint_save=$checkpoint"
    run_stage "imgr10_task_bias_baseline_seed${seed}" --set dual_mask_task_bias_calibration=false \
        --set max_tasks=3 --set "task0_checkpoint_resume=$checkpoint"
    run_stage "imgr10_task_bias_candidate_seed${seed}" --set dual_mask_task_bias_calibration=true \
        --set max_tasks=3 --set "task0_checkpoint_resume=$checkpoint"
done
