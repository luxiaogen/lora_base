...
if [[ "${1:-}" == "--smoke" ]]; then
    shift
    exec bash scripts/9_26_imgr10_old_competition_3090.sh --smoke "$@" --set old_competition_detach_old=true
fi
python -m unittest test.test_old_competition test.test_old_competition_detach test.test_stage_audit || exit 1
bash "$SCRIPT_PATH" --smoke "$@" || exit 1
python scripts/night_mechanism_preflight.py scripts/sweeps/imgr10_old_reference_3090.json || exit 1
echo "Code revision: $(git rev-parse --short HEAD)"
git status --short
LOG_DIR=logs/shell_logs/imgr10_old_reference_3090
TIMESTAMP="$(date +"%Y%m%d_%H%M%S")_full"
FAILED=0
mkdir -p "$LOG_DIR"

echo "============================================================"
echo "Starting imgr10_t10_old_reference_seed1993"
echo "Changed: only detach the old-score reference; competition weight remains 0.1"
echo "Log: $LOG_DIR/imgr10_t10_old_reference_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_old_reference_seed1993_3090_${TIMESTAMP}" \
        ...
        --set wandb_group=imgr10_old_reference_3090 \
        ...
        --set old_competition_weight=0.1 \
        --set old_competition_detach_old=true \
        "$@" \
        2>&1 | tee "$LOG_DIR/imgr10_t10_old_reference_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_t10_old_reference_seed1993"
else
    echo "FAIL imgr10_t10_old_reference_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Finished 1 runs for imgr10_old_reference_3090; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
if [[ "$FAILED" -eq 0 ]]; then
    python scripts/summarize_stage_audit.py --tasks 10 --output "$LOG_DIR/stages_${TIMESTAMP}.csv" \
        "$LOG_DIR/imgr10_t10_old_reference_seed1993_${TIMESTAMP}.log" || FAILED=1
fi
exit $FAILED
