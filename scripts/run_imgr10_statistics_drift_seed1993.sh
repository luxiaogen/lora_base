...
# --smoke runs only the focused diagnostics tests.
case "${1:-}" in
    ""|--smoke) ;;
...
PYTHONWARNINGS=ignore python -m unittest test.test_statistics_drift_diagnostics || exit 1
if [[ "${1:-}" == "--smoke" ]]; then
    exit 0
fi
echo "Code revision: $(git rev-parse --short HEAD)"
LOG_DIR=logs/shell_logs/imgr10_statistics_drift_seed1993
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
FAILED=0
mkdir -p "$LOG_DIR"

echo "============================================================"
echo "Starting imgr10_statistics_drift_seed1993"
echo "Changed: Current baseline with read-only old-statistics drift diagnostics"
echo "Log: $LOG_DIR/imgr10_statistics_drift_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set prefix=imgr10_statistics_drift_seed1993 \
        --set init_epoch=20 \
        --set epochs=20 \
        --set rank=64 \
        --set ca=true \
        ...
        --set dual_mask_statistics_drift_diagnostics=true \
        ...
        --set wandb_group=imgr10_statistics_drift_seed1993 \
        2>&1 | tee "$LOG_DIR/imgr10_statistics_drift_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_statistics_drift_seed1993"
else
    echo "FAIL imgr10_statistics_drift_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Finished 1 runs for imgr10_statistics_drift_seed1993; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
exit $FAILED
