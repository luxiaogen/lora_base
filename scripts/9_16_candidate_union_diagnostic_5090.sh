...
cmd=(
  ...
  --set 'seed=[1993]'
  --set prefix=imgr20_candidate_union_diagnostic_seed1993
  --set max_tasks=20
  ...
  --set wandb_group=candidate_union_diagnostic_5090
  --set wandb_tags=imgr20,t20,qkv,seed1993,candidate_union,evidence_gain,diagnostic
)
...
python -m unittest \
  test.test_predicted_onehot \
  test.test_p_conflict_diagnostics \
  test.test_candidate_union_diagnostic_scripts
python -c 'import json, pathlib; p=pathlib.Path(json.load(open("exps/dlora/imgr20.json"))["data_path"]); print("Dataset:", p); assert (p/"train").is_dir() and (p/"test").is_dir(), "Missing train/ or test/"'
"${cmd[@]}"
