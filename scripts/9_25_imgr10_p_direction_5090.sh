...
if [[ "${1:-}" == "--smoke" ]]; then
    ...
    python -m unittest test.test_p_step_direction test.test_p_step_plot || exit 1
    bash "$SCRIPT_PATH" --smoke "$@" || exit 1
fi
python scripts/night_mechanism_preflight.py scripts/sweeps/imgr10_p_direction_5090.json || exit 1
echo "Code revision: $(git rev-parse --short HEAD)"
git status --short
LOG_DIR=logs/shell_logs/imgr10_p_direction_5090
...
echo "Changed: original SGD; read-only candidate probes"
echo "Log: $LOG_DIR/imgr10_t10_baseline_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_baseline_seed1993_5090_${TIMESTAMP}" \
        ...
        --set wandb_group=imgr10_p_direction_5090 \
        ...
        --set p_step_interval=5 \
        --set p_step_direction=baseline \
        "$@" \
...
echo "============================================================"
echo "Starting imgr10_t10_norm_seed1993"
echo "Changed: P Gram direction; effective QKV norm matched"
echo "Log: $LOG_DIR/imgr10_t10_norm_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_norm_seed1993_5090_${TIMESTAMP}" \
        --set dataset=ImageNet_R \
        --set memory_size=0 \
        --set memory_per_class=0 \
        --set fixed_memory=true \
        ...
        --set task0_margin=0.1 \
        --set p_step_interval=5 \
        --set p_step_direction=norm \
        "$@" \
        2>&1 | tee "$LOG_DIR/imgr10_t10_norm_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_t10_norm_seed1993"
else
    echo "FAIL imgr10_t10_norm_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Starting imgr10_t10_conflict_seed1993"
echo "Changed: same proposals plus actual conflict-update bound"
echo "Log: $LOG_DIR/imgr10_t10_conflict_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_conflict_seed1993_5090_${TIMESTAMP}" \
        --set dataset=ImageNet_R \
        --set memory_size=0 \
        --set memory_per_class=0 \
        --set fixed_memory=true \
...
echo "============================================================"
echo "Finished 3 runs for imgr10_p_direction_5090; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
if [[ "$RUN_KIND" == "smoke" ]]; then
    python scripts/check_p_step_smoke.py \
        "$LOG_DIR/imgr10_t10_baseline_seed1993_${TIMESTAMP}.log" \
        "$LOG_DIR/imgr10_t10_norm_seed1993_${TIMESTAMP}.log" \
        "$LOG_DIR/imgr10_t10_conflict_seed1993_${TIMESTAMP}.log" || exit 1
else
    python scripts/plot_p_step_direction.py \
        "$LOG_DIR/imgr10_t10_baseline_seed1993_${TIMESTAMP}.log" \
        "$LOG_DIR/imgr10_t10_norm_seed1993_${TIMESTAMP}.log" \
        "$LOG_DIR/imgr10_t10_conflict_seed1993_${TIMESTAMP}.log" \
        --out "$LOG_DIR/figures_${TIMESTAMP}" || FAILED=1
fi
exit $FAILED
