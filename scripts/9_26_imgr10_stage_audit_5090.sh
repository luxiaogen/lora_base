...
if [[ "${1:-}" == "--smoke" ]]; then
    ...
    python -m unittest test.test_stage_audit || exit 1
    bash "$SCRIPT_PATH" --smoke "$@" || exit 1
fi
python scripts/night_mechanism_preflight.py scripts/sweeps/imgr10_stage_audit_5090.json || exit 1
echo "Code revision: $(git rev-parse --short HEAD)"
git status --short
LOG_DIR=logs/shell_logs/imgr10_stage_audit_5090
TIMESTAMP="$(date +"%Y%m%d_%H%M%S")_${RUN_KIND}"
FAILED=0
mkdir -p "$LOG_DIR"

echo "============================================================"
echo "Starting imgr10_t10_anchor10_seed1993"
echo "Changed: baseline stage audit; anchor10"
echo "Log: $LOG_DIR/imgr10_t10_anchor10_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_anchor10_seed1993_5090_${TIMESTAMP}" \
        ...
        --set wandb_group=imgr10_stage_audit_5090 \
        ...
        "$@" \
        2>&1 | tee "$LOG_DIR/imgr10_t10_anchor10_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_t10_anchor10_seed1993"
else
    echo "FAIL imgr10_t10_anchor10_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Starting imgr10_t10_anchor5_seed1993"
echo "Changed: only Task0 anchor10 to anchor5"
echo "Log: $LOG_DIR/imgr10_t10_anchor5_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_anchor5_seed1993_5090_${TIMESTAMP}" \
        --set dataset=ImageNet_R \
        --set memory_size=0 \
        --set memory_per_class=0 \
        --set fixed_memory=true \
        ...
        --set p_step_direction=off \
        "$@" \
        2>&1 | tee "$LOG_DIR/imgr10_t10_anchor5_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_t10_anchor5_seed1993"
else
    echo "FAIL imgr10_t10_anchor5_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Finished 2 runs for imgr10_stage_audit_5090; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
if [[ "$FAILED" -eq 0 ]]; then
    python scripts/summarize_stage_audit.py --tasks "$TASKS" --output "$LOG_DIR/stages_${TIMESTAMP}.csv" \
        "$LOG_DIR/imgr10_t10_anchor10_seed1993_${TIMESTAMP}.log" \
        "$LOG_DIR/imgr10_t10_anchor5_seed1993_${TIMESTAMP}.log" || FAILED=1
fi
exit $FAILED
