...
LOG_DIR=logs/shell_logs/oracle_partition_3090_overnight
...
run_job() {
    ...
    if
        python main.py --config "$config" \
            ...
            --set wandb_group=oracle_partition_3090_overnight \
            --set wandb_tags="$protocol,qkv,seed${seed},$repeat,oracle_partition,diagnostic_only" \
            2>&1 | tee "$log_file"
    then
        echo "PASS $name"
    else
