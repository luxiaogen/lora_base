...
tests=(test.test_p_only_after_task0 test.test_p_only_script test.test_task0_trainer test.test_private_rank test.test_ca_diagnostics)
if [[ ${1:-} == --check ]]; then
...
[[ $# == 0 ]] || { echo 'Usage: bash scripts/9_12_p_only_after_task0.sh [--check|--dry-run]' >&2; exit 2; }
seed=1993
if $dry_run; then
    run_dir="logs/shell_logs/p_only_after_task0_DRY_RUN"
else
    ...
    run_dir=$(mktemp -d "logs/shell_logs/p_only_after_task0_$(date +%Y%m%d_%H%M%S)_XXXXXX")
fi
...
common=(--config exps/dlora/imgr10.json
    ...
    --set "wandb_group=imgr10_p_only_after_task0"
    --set "wandb_tags=imgr10,p_only_after_task0,ca5,partial_task2")
...
checkpoint="$run_dir/task0_seed1993.pt"
run_stage imgr10_task0_seed1993 \
    --set dual_mask_s_task0_only=false --set max_tasks=1 --set "task0_checkpoint_save=$checkpoint"
run_stage imgr10_sp_baseline_seed1993 \
    --set dual_mask_s_task0_only=false --set max_tasks=3 --set "task0_checkpoint_resume=$checkpoint"
run_stage imgr10_p_only_seed1993 \
    --set dual_mask_s_task0_only=true --set max_tasks=3 --set "task0_checkpoint_resume=$checkpoint"
