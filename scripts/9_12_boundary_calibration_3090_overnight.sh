...
tests=(test.test_boundary_calibration test.test_task0_checkpoint test.test_overnight_scripts)
if [[ ${1:-} == --smoke ]]; then
...
[[ $# == 0 ]] || { echo 'Usage: bash scripts/9_12_boundary_calibration_3090_overnight.sh [--smoke|--dry-run]' >&2; exit 2; }

if $dry_run; then
    run_dir=logs/shell_logs/boundary_calibration_3090_DRY_RUN
else
    ...
    run_dir=$(mktemp -d "logs/shell_logs/boundary_calibration_3090_$(date +%Y%m%d_%H%M%S)_XXXXXX")
fi
...
common=(--config exps/dlora/imgr10.json
    ...
    --set dual_mask_previous_function_enabled=false --set dual_mask_previous_function_weight=1.0
    ...
    --set wandb_group=imgr10_boundary_calibration_3090_overnight)
...
for seed in 1993 1996 1997; do
    ...
    run_stage "$seed" "imgr10_boundary_task0_seed${seed}" \
        --set dual_mask_boundary_calibration=false --set max_tasks=1 \
        --set "task0_checkpoint_save=$checkpoint" \
        --set wandb_tags=imgr10,boundary_calibration,task0,paired,ca5,seed${seed}
    if [[ $seed == 1993 ]]; then
        run_stage "$seed" "imgr10_boundary_screen_baseline_seed${seed}" \
            --set dual_mask_boundary_calibration=false --set max_tasks=3 \
            --set "task0_checkpoint_resume=$checkpoint" \
            --set wandb_tags=imgr10,boundary_calibration,screen,baseline,paired,ca5,seed${seed}
        run_stage "$seed" "imgr10_boundary_screen_candidate_seed${seed}" \
            --set dual_mask_boundary_calibration=true --set max_tasks=3 \
            --set "task0_checkpoint_resume=$checkpoint" \
            --set wandb_tags=imgr10,boundary_calibration,screen,candidate,paired,ca5,seed${seed}
    fi
    run_stage "$seed" "imgr10_boundary_full_baseline_seed${seed}" \
        --set dual_mask_boundary_calibration=false --set max_tasks=10 \
        --set "task0_checkpoint_resume=$checkpoint" \
        --set wandb_tags=imgr10,boundary_calibration,full,baseline,paired,ca5,seed${seed}
    run_stage "$seed" "imgr10_boundary_full_candidate_seed${seed}" \
        --set dual_mask_boundary_calibration=true --set max_tasks=10 \
        --set "task0_checkpoint_resume=$checkpoint" \
        --set wandb_tags=imgr10,boundary_calibration,full,candidate,paired,ca5,seed${seed}
done

echo '============================================================'
echo "Finished 11 stages for boundary-calibration 3090 overnight; FAILED=$FAILED"
