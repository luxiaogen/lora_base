...
tests=(test.test_boundary_calibration test.test_boundary_real_current_script test.test_task0_checkpoint)
if [[ ${1:-} == --smoke ]]; then
...
[[ $# == 0 ]] || { echo 'Usage: bash scripts/9_12_boundary_real_current_screen.sh [--smoke|--dry-run]' >&2; exit 2; }

if $dry_run; then
    run_dir=logs/shell_logs/boundary_real_current_DRY_RUN
else
    ...
    run_dir=$(mktemp -d "logs/shell_logs/boundary_real_current_$(date +%Y%m%d_%H%M%S)_XXXXXX")
fi
...
common=(--config exps/dlora/imgr10.json
    ...
    --set "wandb_mode=${WANDB_MODE:-online}"
    --set wandb_group=imgr10_boundary_real_current_screen)
...
run_stage "imgr10_boundary_real_current_task0_seed${SEED}" \
    --set dual_mask_boundary_calibration=false \
    --set dual_mask_boundary_real_current=false \
    --set max_tasks=1 \
    --set "task0_checkpoint_save=$checkpoint" \
    --set wandb_tags=imgr10,boundary_real_current,task0,paired,ca5,seed${SEED}
run_stage "imgr10_boundary_real_current_baseline_seed${SEED}" \
    --set dual_mask_boundary_calibration=false \
    --set dual_mask_boundary_real_current=false \
    --set max_tasks=3 \
    --set "task0_checkpoint_resume=$checkpoint" \
    --set wandb_tags=imgr10,boundary_real_current,screen,baseline,paired,ca5,seed${SEED}
run_stage "imgr10_boundary_real_current_candidate_seed${SEED}" \
    --set dual_mask_boundary_calibration=true \
    --set dual_mask_boundary_real_current=true \
    --set max_tasks=3 \
    --set "task0_checkpoint_resume=$checkpoint" \
    --set wandb_tags=imgr10,boundary_real_current,screen,candidate,paired,ca5,seed${SEED}

echo "Finished paired Task1-2 screen (NOT a full T10 experiment). Logs/checkpoint: $run_dir"
