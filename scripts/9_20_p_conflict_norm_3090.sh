...
LOG_DIR=logs/shell_logs/p_conflict_norm_screen
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
FAILED=0
mkdir -p "$LOG_DIR"

echo "============================================================"
echo "Starting imgr10_norm_seed1993"
echo "Changed: P-conflict norm regularizer; no historical feature statistics"
echo "Log: $LOG_DIR/imgr10_norm_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set prefix=imgr10_norm_seed1993 \
        --set init_cls=20 \
        --set increment=20 \
        ...
        --set wandb_group=p_conflict_norm_screen \
        --set use_slora=true \
        --set use_plora=true \
        ...
        --set dual_mask_p_input_subspace=norm \
        2>&1 | tee "$LOG_DIR/imgr10_norm_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_norm_seed1993"
else
    echo "FAIL imgr10_norm_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Finished 1 runs for p_conflict_norm_screen; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
exit $FAILED
