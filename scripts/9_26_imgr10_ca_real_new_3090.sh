...
python scripts/night_mechanism_preflight.py scripts/sweeps/imgr10_ca_real_new_3090.json || exit 1
echo "Code revision: $(git rev-parse --short HEAD)"
git status --short
LOG_DIR=logs/shell_logs/imgr10_ca_real_new_3090
TIMESTAMP="$(date +"%Y%m%d_%H%M%S")_${RUN_KIND}"
FAILED=0
mkdir -p "$LOG_DIR"

echo "============================================================"
echo "Starting imgr10_t10_gaussian_seed1993"
echo "Changed: baseline CA all Gaussian"
echo "Log: $LOG_DIR/imgr10_t10_gaussian_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_gaussian_seed1993_3090_${TIMESTAMP}" \
        ...
        --set wandb_group=imgr10_ca_real_new_3090 \
        ...
        "$@" \
        2>&1 | tee "$LOG_DIR/imgr10_t10_gaussian_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_t10_gaussian_seed1993"
else
    echo "FAIL imgr10_t10_gaussian_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Starting imgr10_t10_real_new_seed1993"
echo "Changed: only replace new-class CA samples with current real features"
echo "Log: $LOG_DIR/imgr10_t10_real_new_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_real_new_seed1993_3090_${TIMESTAMP}" \
        --set dataset=ImageNet_R \
        --set memory_size=0 \
        --set memory_per_class=0 \
        --set fixed_memory=true \
        ...
        --set old_competition_weight=0 \
        --set old_competition_detach_old=false \
        --set ca_real_new_features=true \
        "$@" \
        2>&1 | tee "$LOG_DIR/imgr10_t10_real_new_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_t10_real_new_seed1993"
else
    echo "FAIL imgr10_t10_real_new_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Finished 2 runs for imgr10_ca_real_new_3090; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
if [[ "$FAILED" -eq 0 ]]; then
