...
python -m unittest test.test_mask_reg_ablation test.test_branch_reg_ablation || exit 1
python scripts/night_mechanism_preflight.py scripts/sweeps/imgr10_branch_reg_5090.json || exit 1
echo "Code revision: $(git rev-parse --short HEAD)"
git status --short
LOG_DIR=logs/shell_logs/imgr10_branch_reg_5090
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
FAILED=0
mkdir -p "$LOG_DIR"

echo "============================================================"
echo "Starting imgr10_t10_both_seed1993"
echo "Changed: A: retain S and P mask penalties"
echo "Log: $LOG_DIR/imgr10_t10_both_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_both_seed1993_${TIMESTAMP}" \
        ...
        --set wandb_group=imgr10_branch_reg_5090 \
        --set plora_lr_multiplier=1 \
        --set dual_mask_reg_grad_diagnostic=true \
        --set dual_mask_s_reg_enabled=true \
        --set dual_mask_p_reg_enabled=true \
        "$@" \
        2>&1 | tee "$LOG_DIR/imgr10_t10_both_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_t10_both_seed1993"
else
    echo "FAIL imgr10_t10_both_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Starting imgr10_t10_s_only_seed1993"
echo "Changed: C: retain S penalty, zero P penalty"
echo "Log: $LOG_DIR/imgr10_t10_s_only_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_s_only_seed1993_${TIMESTAMP}" \
        --set dataset=ImageNet_R \
        --set memory_size=0 \
        --set memory_per_class=0 \
        --set fixed_memory=true \
        ...
        --set dual_mask_reg_grad_diagnostic=true \
        --set dual_mask_s_reg_enabled=true \
        --set dual_mask_p_reg_enabled=false \
        "$@" \
        2>&1 | tee "$LOG_DIR/imgr10_t10_s_only_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_t10_s_only_seed1993"
else
    echo "FAIL imgr10_t10_s_only_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Starting imgr10_t10_p_only_seed1993"
echo "Changed: D: zero S penalty, retain P penalty"
echo "Log: $LOG_DIR/imgr10_t10_p_only_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_p_only_seed1993_${TIMESTAMP}" \
        --set dataset=ImageNet_R \
        --set memory_size=0 \
        --set memory_per_class=0 \
        --set fixed_memory=true \
        ...
        --set plora_lr_multiplier=1 \
        --set dual_mask_reg_grad_diagnostic=true \
        --set dual_mask_s_reg_enabled=false \
        --set dual_mask_p_reg_enabled=true \
        "$@" \
        2>&1 | tee "$LOG_DIR/imgr10_t10_p_only_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_t10_p_only_seed1993"
else
    echo "FAIL imgr10_t10_p_only_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Finished 3 runs for imgr10_branch_reg_5090; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
exit $FAILED
