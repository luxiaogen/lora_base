...
python -m unittest test.test_p_conflict_diagnostics test.test_qk_all_tasks test.test_qv_after_task0 || exit 1
...
git status --short
LOG_DIR=logs/shell_logs/imgr10_soft_qk_full_t10_5090
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
FAILED=0
mkdir -p "$LOG_DIR"

echo "============================================================"
echo "Starting imgr10_soft_qkv_seed1993"
echo "Changed: Fresh QKV; same-model ones/uniform/soft/oracle after CA"
echo "Log: $LOG_DIR/imgr10_soft_qkv_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set prefix=imgr10_soft_qkv_seed1993 \
        --set max_tasks=10 \
        ...
        --set wandb_group=imgr10_soft_qkv_full_t10_5090 \
        --set wandb_tags=imgr10,t10,qkv,p_conflict_soft,inference_only,multiseed \
        2>&1 | tee "$LOG_DIR/imgr10_soft_qkv_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_soft_qkv_seed1993"
else
    echo "FAIL imgr10_soft_qkv_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Starting imgr10_qk_all_seed1993"
echo "Changed: Fresh QK in Task0-9; same read-only diagnostics as QKV"
...
if
    python main.py --config exps/dlora/imgr10.json \
        ...
        --set dual_mask_p_conflict_diagnostics=true \
        --set wandb_group=imgr10_qk_all_full_t10_5090 \
...
echo "============================================================"
echo "Starting imgr10_soft_qkv_seed1996"
echo "Changed: Fresh QKV; same-model ones/uniform/soft/oracle after CA"
echo "Log: $LOG_DIR/imgr10_soft_qkv_seed1996_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1996]' \
        --set prefix=imgr10_soft_qkv_seed1996 \
        --set max_tasks=10 \
        --set total_sessions=10 \
        --set init_cls=20 \
        --set increment=20 \
        ...
        --set wandb_group=imgr10_soft_qkv_full_t10_5090 \
        --set wandb_tags=imgr10,t10,qkv,p_conflict_soft,inference_only,multiseed \
        2>&1 | tee "$LOG_DIR/imgr10_soft_qkv_seed1996_${TIMESTAMP}.log"
then
    echo "PASS imgr10_soft_qkv_seed1996"
else
    echo "FAIL imgr10_soft_qkv_seed1996"
    FAILED=1
fi
...
echo "============================================================"
echo "Starting imgr10_soft_qkv_seed1997"
echo "Changed: Fresh QKV; same-model ones/uniform/soft/oracle after CA"
echo "Log: $LOG_DIR/imgr10_soft_qkv_seed1997_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1997]' \
        --set prefix=imgr10_soft_qkv_seed1997 \
        --set max_tasks=10 \
        --set total_sessions=10 \
        --set init_cls=20 \
        --set increment=20 \
        ...
        --set wandb_group=imgr10_soft_qkv_full_t10_5090 \
        --set wandb_tags=imgr10,t10,qkv,p_conflict_soft,inference_only,multiseed \
        2>&1 | tee "$LOG_DIR/imgr10_soft_qkv_seed1997_${TIMESTAMP}.log"
then
    echo "PASS imgr10_soft_qkv_seed1997"
else
    echo "FAIL imgr10_soft_qkv_seed1997"
    FAILED=1
fi
...
echo "============================================================"
echo "Finished 6 runs for imgr10_soft_qk_full_t10_5090; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
exit $FAILED
