...
python scripts/night_mechanism_preflight.py scripts/sweeps/imgr10_old_competition_5090.json || exit 1
echo "Code revision: $(git rev-parse --short HEAD)"
git status --short
LOG_DIR=logs/shell_logs/imgr10_old_competition_5090
...
if
    python main.py --config exps/dlora/imgr10.json \
        ...
        --set wandb_group=imgr10_old_competition_5090 \
...
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_competition_seed1993_5090_${TIMESTAMP}" \
        --set dataset=ImageNet_R \
        --set memory_size=0 \
        --set memory_per_class=0 \
        --set fixed_memory=true \
...
echo "============================================================"
echo "Finished 2 runs for imgr10_old_competition_5090; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
if [[ "$FAILED" -eq 0 ]]; then
