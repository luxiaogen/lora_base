...
cd "$(dirname "$0")/.."
LOG_DIR=logs/shell_logs/imgr10_p_conflict_off_smoke
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
FAILED=0
mkdir -p "$LOG_DIR"

echo "============================================================"
echo "Starting imgr10_b_p_conflict_off_smoke_seed1993"
echo "Changed: One-epoch route smoke for B; not a performance result"
echo "Log: $LOG_DIR/imgr10_b_p_conflict_off_smoke_seed1993_${TIMESTAMP}.log"
echo "============================================================"
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set prefix=imgr10_b_p_conflict_off_smoke_seed1993 \
        --set init_epoch=1 \
        --set epochs=1 \
        --set rank=64 \
        --set ca=false \
        ...
        2>&1 | tee "$LOG_DIR/imgr10_b_p_conflict_off_smoke_seed1993_${TIMESTAMP}.log"
then
    echo "PASS imgr10_b_p_conflict_off_smoke_seed1993"
else
    echo "FAIL imgr10_b_p_conflict_off_smoke_seed1993"
    FAILED=1
fi

echo "============================================================"
echo "Finished 1 runs for imgr10_p_conflict_off_smoke; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
exit $FAILED
