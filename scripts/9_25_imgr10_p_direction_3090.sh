...
python scripts/night_mechanism_preflight.py scripts/sweeps/imgr10_p_direction_3090.json || exit 1
echo "Code revision: $(git rev-parse --short HEAD)"
git status --short
LOG_DIR=logs/shell_logs/imgr10_p_direction_3090
...
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_baseline_seed1993_3090_${TIMESTAMP}" \
        ...
        --set wandb_group=imgr10_p_direction_3090 \
...
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_norm_seed1993_3090_${TIMESTAMP}" \
        --set dataset=ImageNet_R \
        --set memory_size=0 \
        --set memory_per_class=0 \
        --set fixed_memory=true \
...
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_conflict_seed1993_3090_${TIMESTAMP}" \
        --set dataset=ImageNet_R \
        --set memory_size=0 \
        --set memory_per_class=0 \
        --set fixed_memory=true \
...
echo "============================================================"
echo "Finished 3 runs for imgr10_p_direction_3090; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
