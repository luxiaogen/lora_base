...
python scripts/night_mechanism_preflight.py scripts/sweeps/imgr10_stage_audit_3090.json || exit 1
echo "Code revision: $(git rev-parse --short HEAD)"
git status --short
LOG_DIR=logs/shell_logs/imgr10_stage_audit_3090
...
if
    python main.py --config exps/dlora/imgr10.json \
        --set 'seed=[1993]' \
        --set "prefix=imgr10_t10_anchor10_seed1993_3090_${TIMESTAMP}" \
        ...
        --set wandb_group=imgr10_stage_audit_3090 \
...
echo "============================================================"
echo "Finished 1 runs for imgr10_stage_audit_3090; FAILED=$FAILED"
echo "Logs: $LOG_DIR"
echo "============================================================"
if [[ "$FAILED" -eq 0 ]]; then
    python scripts/summarize_stage_audit.py --tasks "$TASKS" --output "$LOG_DIR/stages_${TIMESTAMP}.csv" \
        "$LOG_DIR/imgr10_t10_anchor10_seed1993_${TIMESTAMP}.log" || FAILED=1
fi
exit $FAILED
