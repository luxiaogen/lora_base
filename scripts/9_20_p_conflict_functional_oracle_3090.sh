...
# Run from the repository root, including through lrun. data_path stays in the local JSON.
...
LOG_DIR=logs/shell_logs/p_conflict_functional_oracle
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
RUN_NAME="imgr10_p_conflict_functional_oracle_seed1993_${TIMESTAMP}"
...
echo "Diagnostic: Layer x QKV x 8 rank groups; disjoint label selector/evaluator"
echo "Labels are diagnostic-only and never change the merged model."
echo "Log: $RUN_LOG"
echo "============================================================"
...
if
    python main.py --config exps/dlora/imgr10.json \
        ...
        --set wandb_group=p_conflict_functional_oracle \
        --set wandb_tags=imagenet_r,t10,seed1993,p_conflict,functional_oracle,diagnostic \
        2>&1 | tee "$RUN_LOG"
then
    echo "PASS $RUN_NAME"
    exit 0
