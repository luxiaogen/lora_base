...
class Attention_LoRA(nn.Module):
    ...
    def before_task(self, task: int):
        ...
        projection_mode = "QKV"
        if self.args.get("dual_mask_qv_all_tasks", False):
            projection_mode = "QV"
        elif self.args.get("dual_mask_qk_all_tasks", False) or (t == 0 and self.args.get("dual_mask_task0_qk", False)):
            projection_mode = "QK"
    ...
    def _projection_delta(self, delta: torch.Tensor) -> torch.Tensor:
        ...
        elif self.args.get("dual_mask_qk_all_tasks", False) or (self.cur_task == 0 and self.args.get("dual_mask_task0_qk", False)):
            delta = delta.clone()
